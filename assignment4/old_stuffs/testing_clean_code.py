...
class GridThing():
    ...
    def greens_calc(self, walk_numb, start_value):
        ...
        global_walk_counts = self.grid_array[1:self.n_val+1, 1:self.n_val+1]*0
        boundry_counts = global_walk_counts*0
        ...
        for _ in range (0, walk_numb):
            ...
            walker_counts = global_walk_counts*0
            ...
            global_walk_counts += walker_counts
            boundry_counts[position[0], position[1]] += 1
        ...
        greens_function = self.h_val**2*global_walk_counts/walk_numb
        greens_function_laplace = boundry_counts/walk_numb
        # Calculating varience arrays

        return greens_function, greens_function_laplace


    def function_val(self, greens_function, green_laplace, function):
        '''
        ...
        potential_grid = self.grid_array[1:self.n_val+1,1:self.n_val+1]*1
        potential_grid = function(x_outer_product,
                                  y_outer_product)

        part1 = np.sum(function(x_outer_product, y_outer_product)*greens_function)
        boundry_vals = self.boundryfixed()
        part2 = np.sum(boundry_vals*green_laplace)
        ...
        self.temp2 = green_laplace*boundry_vals
    ...
    def boundryfixed(self):
        array = self.grid_array[1:self.n_val+1, 1:self.n_val+1]*0
        # top
        array[0, 1: -1] = self.grid_array[0, 2:-2]
        # bottem
        array[-1, 1: -1] = self.grid_array[-1, 2:-2]
        # left
        array[1:-1, 0] = self.grid_array[2:-2, 0]
        # right
        array[1:-1, -1] = self.grid_array[2:-2, -1]        
        return array
...
def troubles_func(greens_potential_array, val_relaxed, n_val, laplace_array):
    
    possion_contribution = np.sum(greens_potential_array[1:n_val+1,1:n_val+1])
    print('---\nposssion part:', possion_contribution)
    true_contribution = val_relaxed - np.sum(laplace_array)
...
# green_array, green_laplace = a.greens_calc(100, point_of_interest)
# c = a.function_val(green_array, green_laplace, function_a)

# print('actual:', c0)
# print('walker:', c)

# troubles_func(a.temp, c0, N_val, a.temp2)
